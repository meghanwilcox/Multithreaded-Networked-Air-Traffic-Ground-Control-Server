...
static void *takeoff_thread(void *arg){
    ...
    while(1){
        char* flightid = takeoff_list_get(0);
        if(flightid != NULL){
            airplane* plane = planelist_find(flightid);
            if(plane != NULL){
                if(plane->state == PLANE_INAIR){
                    printf("Flight %s is in the air - waiting 4 seconds", flightid);
                    takeoff_list_remove(flightid);
                    plane->state = PLANE_DONE;
                    sleep(4);;
                }
                else if(plane->state == PLANE_TAXIING){
...
static void *client_thread(void *arg) {
    ...
    while (myplane->state != PLANE_DONE) {
        ...
        // if(myplane->state == PLANE_CLEAR){
        //     printf("takeoff\n");
        //     fprintf(myplane->fp_send, "TAKEOFF");
        // }
    }
...
int main(int argc, char *argv[]) {
    ...
    pthread_t takeoff_thread_name;
    pthread_create(&takeoff_thread_name, NULL, takeoff_thread, NULL);
